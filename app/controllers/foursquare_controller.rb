class FoursquareController < ApplicationController
  ...
  def push
  	if !params[:secret].nil? && (params[:secret] == FoursquareAccount::FOURSQUARE_PUSH_SECRET)
  		foursquare_account = FoursquareAccount.find_by_foursquare_id(JSON.parse(params[:checkin])["user"]["id"])
  		if !foursquare_account.nil? && foursquare_account.active
  			FoursquareCheckin.create(:foursquare_account_id => foursquare_account.id, :venue_id => JSON.parse(params[:checkin])["venue"]["id"], :venue_name => JSON.parse(params[:checkin])["venue"]["name"])
  		end
  	end
  	render :nothing => true
  end
